...
async function all_jobs(url = 'limit=50'){
    const res = await fetch("https://remotive.com/api/remote-jobs?" + url)
    const resoult = await res.json()
    const data = await resoult.jobs
    return data
}
...
async function creatcard(url = 'limit=50'){
    const arr =await all_jobs(url)
  console.log(arr)
    main.innerHTML = ''
    arr.forEach((element) => {
    main.innerHTML += `
...
async function create_category(){
  ...
  arr.forEach(item => {
    dropdown.innerHTML += `<li><button class="dropdown-item" onclick="creatcard('category=${item.slug}&limit=50')">${item.name}</button></li>`
  })
...
async function to_save(id) {
  const data = await all_jobs()
  data.forEach(item => {
    if (item.id === id) {
